...
public class NCPassProtocol {
    ...
    public NCPassProtocol() {
        ...
        transID = String.format("%06d", generator.nextInt(1000000));
    }
     ...
     */
    String authenticationCode(int code) {
        switch (code) {
            case 0:
    ...
    public Hashtable decodeHandshake(byte[] handshake) throws UnsupportedEncodingException {
        Hashtable handshakeResult = decodeHeader(handshake);
        int length = 0;
        int c = 14;
    ...
    /**
     * Decodes the NCPASS query response and returns a hashtable containing all of the returned data fields. Also contains text explanations of the Validation and AuthenticationResult codes
     * 
    ...
    public Hashtable decodeResponse(byte[] response) throws UnsupportedEncodingException {
        Hashtable requestResult = decodeHeader(response);
        int length = 0;
        int c = 14;
        ...
        if (length != 0) {
            ...
            c++; // if no message field, there will be an extra character to bypass
        }
        c = c + length;
        length = getParameterHex(response[c++], response[c++]);
        if (length != 0) {
     ...
     */
    String validationCode(int code) {
        switch (code) {
            case 0:
